...
function TodoInput({isCompleted, todo, toggle, toUpdate, handleUpdateExpand, todoNum}) {
    ...
    return (
        <motion.div layout key="TodoInput" variants={toggle} initial='hidden' animate='vissible' exit="hidden" className="relative h-auto w-auto p-4 text-black rounded-lg shadow-sm border-2 border-gray-300 mb-5">
            ...
            {setTodoLoading ? <Loader/> : <div className="mt-5 flex items-center space-x-3">
                {toUpdate && isUpdating ? <Loader/> :<button onClick={toUpdate ? handleUpdateTodo : addTodo} className=" hover:scale-95 transform transition-all text-white bg-blue-500 text-sm font-semibold border-none rounded-full px-4 py-2">{toUpdate ? "Update Todo" : "Add Todo"}</button>}

        ...
        <section className=" md:w-full w-screen min-h-screen relative flex md:items-center justify-center ">
            ...
            <motion.div layout className="relative md:w-1/2 w-full h-auto border-2 border-gray-100 rounded-lg shadow-lg px-2 py-4">
                    <motion.h1 layout className="relative  flex items-center justify-center text-center font-semibold text-xl mb-8 py-3">Real Time Todo <button onClick={openInputBox} className="absolute h-12 w-12  right-0 top-1/2 transform -translate-y-1/2 rounded-3xl bg-blue-500 grid place-content-center">
                        <FaPlus className={`${isOpen && "rotate-45 duration-300"} active:scale-95 transform text-white font-semibold text-lg transition-transform`} /></button>
                        <FaSignOutAlt onClick={()=>logout(dispatch)} aria-label="logout button" className="hover:scale-110 absolute top-1/2 transform -translate-y-1/2 left-0 text-4xl cursor-pointer text-red-500"/>
                    </motion.h1>
                <AnimatePresence>
                        {isOpen &&
                    ...
                    <motion.p layout className="flex px-2 space-x-4 justify-end w-full mt-8">{isDeletingAllTodo ? <Loader /> : <button onClick={deleteAllTodos} className={`${todosDetails.length < 1 && "hidden"} text-white bg-blue-500 px-4 py-1 rounded-full text-sm font-semibold `}>Delete all : {todosDetails.length}</button>} {todosDetails.completedTodos >= 1 && isDeletingCompletedTodo ? <Loader /> : <button onClick={deleteCompletedTodos} className={`${todosDetails.completedTodos <1 && "hidden"} text-white bg-green-500 px-4 py-1 rounded-full text-sm font-semibold `}>Delete completed : {todosDetails.completedTodos}</button>}</motion.p>
